...
describe.only("Rare skills challenges", function () {
  let advancedErc777Contract: any;
  let bundingCurveContract: any;
  let owner: any;
  let account1: any;
  let account2: any;
  let godAccount: any;
  let erc777factory: any;
  let bondingCurveFactory: any;
  ...
  describe("Bonding curve purchase and sale", async () => {
    ...
    it("should confirm that the bonding curve price is correct", async () => {
      ...
      // @ts-ignore
      const balance1: number = Number(ethers.utils.formatEther(userBalance1.toString())).toFixed(0);
      // @ts-ignore
      const balance2: number = Number(ethers.utils.formatEther(userBalance2.toString())).toFixed(0);
